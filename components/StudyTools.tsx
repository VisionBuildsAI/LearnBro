...
import { Check, X, RotateCw, ChevronRight, ChevronLeft, BrainCircuit, AlertCircle, HelpCircle, Eye, EyeOff, Trophy, Sparkles, FileText, Bookmark, Lightbulb, CheckCircle2, Shuffle, Plus, Trash2, Printer, Clock, FileQuestion } from 'lucide-react';
...
export const QuestionPaperView: React.FC<{ data: QuestionPaperData }> = ({ data }) => {
    if (!data || !data.sections) {
        return (
            <div className="p-4 bg-red-50 border border-red-200 rounded-2xl text-red-600 flex items-center gap-2">
                <AlertCircle size={20} />
                <p>Could not load question paper.</p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full bg-white text-slate-900 rounded-2xl border border-slate-200 p-8 shadow-sm print-container font-serif">
            {/* Exam Header */}
            <div className="text-center border-b-2 border-slate-900 pb-6 mb-8">
                <h1 className="text-3xl font-bold uppercase tracking-wide text-slate-900 mb-2">{data.title}</h1>
                <div className="flex justify-between items-center text-sm font-bold text-slate-600 mt-4 px-4">
                    <span className="flex items-center gap-2"><Clock size={16} /> Duration: {data.durationMinutes} Mins</span>
            ...
            {data.instructions && data.instructions.length > 0 && (
                <div className="mb-8 p-4 bg-slate-50 border border-slate-100 rounded-lg">
                    <h3 className="text-sm font-bold uppercase tracking-wider mb-2 text-slate-700">General Instructions:</h3>
                ...
                {data.sections.map((section, idx) => (
                    <div key={idx} className="break-inside-avoid">
                        <div className="flex items-center justify-center mb-6">
                            <h3 className="text-md font-bold text-slate-900 uppercase tracking-widest bg-slate-100 px-4 py-1 rounded-full border border-slate-200">
                                {section.name}
                            </h3>
                        </div>
                        <div className="space-y-6">
                            {section.questions.map((q) => (
                                <div key={q.id} className="flex gap-4 group">
                                    <div className="flex-shrink-0 w-8 font-bold text-slate-500 text-right">{q.id}.</div>
                                    <div className="flex-1">
                                        <p className="text-base text-slate-800 leading-relaxed font-medium">{q.text}</p>
                                    </div>
                                    <div className="flex-shrink-0 text-sm font-bold text-slate-400 w-12 text-right group-hover:text-slate-600 transition-colors">
                                        [{q.marks}]
                                    </div>
                                </div>
                            ))}
            ...
            <div className="mt-12 pt-8 border-t border-slate-200 text-center">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">*** End of Paper ***</p>
                <button 
                    onClick={handlePrint}
                    className="inline-flex items-center gap-2 px-6 py-2 bg-slate-900 text-white rounded-lg text-sm font-bold hover:bg-slate-700 transition-colors print:hidden"
                >
